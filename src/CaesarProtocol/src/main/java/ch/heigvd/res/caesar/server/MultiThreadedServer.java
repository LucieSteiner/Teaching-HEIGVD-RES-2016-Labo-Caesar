	 ...
	 */
		private class ServantWorker implements Runnable {
			...
			public void run() {
				...
				//out.flush();
				try {   LOG.info("Reading messages from client...");
					
					while ((shouldRun) && (line = in.readLine()) != null) {
                                           LOG.log(Level.INFO, "Message from client: {0}", line);
                                           
                                           if(!encrypted){
                                              if(line.equalsIgnoreCase("hello")){
                                                 out.println(key);
                                                 out.flush();
                                                 LOG.log(Level.INFO, "Send key: {0}", key);
                                                 encrypted = true;
                                              }
                                           }
                                           else{
                                                LOG.info("Decrypt messages from client and stop if message is BYE...");
                                                line = Protocol.decrypt(line, key);
                                                LOG.log(Level.INFO, "Decrypted message : {0}", line);
						if (line.equalsIgnoreCase("bye")) {
							shouldRun = false;
						}
                                                LOG.info("Encrypt message and send it back to client...");
						out.println(Protocol.encrypt(line, key));
						out.flush();
                                           }
					}
